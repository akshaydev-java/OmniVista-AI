...
def download_url(url, fname, root=os.path.expanduser('./ckpts')):
...
def download(id_or_url, fname, root=os.path.expanduser('./ckpts')):
    if id_or_url.startswith('http'):
        return download_url(id_or_url, fname, root)
    ...
    response = session.get(URL, params={'id': id_or_url}, stream=True)
    token = get_confirm_token(response)

    if token:
        params = {'id': id_or_url, 'confirm': token}
